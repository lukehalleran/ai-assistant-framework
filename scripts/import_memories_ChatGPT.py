...
class MemoryFactExtractor:
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            (r"(\w+)(?:'s)?\s+(?:cat|dog|pet)(?:'s)?\s+(?:is\s+)?named?\s+(\w+)(?:\.|,|$)", "pet_name", 0.90),  # "Luke's cat is named Biscuit"
            (r"(\w+)\s+(?:has|had)\s+a\s+(cat|dog|pet)\s+named\s+(\w+)", "pet_relationship", 0.88),  # "Luke has a cat named Biscuit"
        ...
        if isinstance(match, tuple):
            if len(match) >= 2:
                ...
                if relation_hint == "fitness_goal":
                    ...
                    # (name, location) - e.g., "Luke is located in Springfield"
                    relation = "lives_in"
                    obj = match[1].strip()
                    ...
                    # (name, pet_type, pet_name) - e.g., "Luke has a cat named Biscuit"
                    pet_type = match[1].lower()
                    relation = f"{pet_type}_name"
                    obj = match[2]
                elif relation_hint == "pet_name" and len(match) == 2:
                    # (owner_name, pet_name) from "Luke's cat is named Biscuit"
                    relation = "pet_name"
                    obj = match[1]
                else:
