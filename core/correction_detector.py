...
- Purpose: Detect when a user corrects or confirms previously stored facts,
  including entity-level corrections (e.g., "Biscuit did not die").
...
class EntityCorrectionEvent(BaseModel):
    """An event indicating an entity-level correction (e.g., 'Biscuit is alive').
...
_ENTITY_CORRECTION_PATTERNS = [
    ...
    # "Biscuit is still with us / still around / still kicking"
    (re.compile(r"\b(?P<entity>[A-Z]\w+)\s+is\s+still\s+(?:with\s+us|around|kicking)\b", re.I), 0.85, "alive"),
    # "no, Biscuit didn't die" / "no Biscuit is alive"
...
class CorrectionDetector:
    ...
    def detect_entity_corrections(
        ...
        """Detect entity-level corrections (e.g., 'Biscuit did not die').
