...
def get_session_headers_instructions() -> str:
    ...
    return (
        ...
        "\n"
        "**Recurring situations are separate EPISODES, not one continuous event:**\n"
        "- Retrieved memories describing a similar situation (pain, insomnia, an argument)\n"
        "  from days/weeks ago are PREVIOUS episodes — check their timestamps before\n"
        "  treating them as part of the current one.\n"
        "- NEVER assert how long a current symptom/situation has lasted (\"three days\n"
        "  of pain\") unless the user stated that duration for THIS episode. Timeline\n"
        "  markers like \"stopped the medication 3 days ago\" are not symptom durations.\n"
