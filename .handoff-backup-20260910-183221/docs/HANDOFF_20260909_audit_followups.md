...
| 11 | **IN TREE, see `HANDOFF_20260910_web_search_gap.md` — Web search silently absent on news shares + confabulated "search came back empty".** | `utils/web_search_trigger.py`, `core/agentic/gate.py`, `core/prompt/gatherer_web.py`, `core/prompt/builder.py`, `core/prompt/formatter.py`, `core/orchestrator.py` | Tests-first receipt and completed implementation are recorded in the handoff Results section; owner/referee review and live post-restart probe remain. |
