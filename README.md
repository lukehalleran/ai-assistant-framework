...
Before retrieval, queries are expanded using a NetworkX knowledge graph built from extracted facts. Entity resolution with alias tables (trigram → bigram → unigram matching) turns "what about my brother" into "what about my brother Sam Mom Biscuit" — surfacing related entities the user didn't explicitly mention. Expansion candidates are ranked by lateral connectivity and filtered for junk nodes.
...
> *Daemon:* Returns comprehensive profile including identity, health history, pets (Biscuit, Mr. Bean's AirTag), fitness PRs, personal relationships, and ongoing projects.

**Emotional context understanding:**
> *User:* "What does Mochi mean to me?"
> *Daemon:* "Mochi was your lifeline... You've told me she saved your life during a period when you were in a really dark place. She gave you a reason to keep going when you couldn't find one yourself."
...
- ChromaDB: ~50MB
- Wikipedia FAISS index + metadata: ~14.5GB (optional, [download from HuggingFace](https://huggingface.co/datasets/MochiLives/daemon-wiki-faiss))
- Logs: ~1MB/day
