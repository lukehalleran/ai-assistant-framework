...
[![Tests](https://img.shields.io/badge/tests-5%2C747-brightgreen.svg)](#testing)
[![Docker Ready](https://img.shields.io/badge/docker-ready-blue.svg)](https://www.docker.com/)

> ~186K lines of Python (≈141K code) across 528 files | 14 ChromaDB collections | ~5,700 tests | 20 agentic tools | solo-built part-time over ~14 months
...
When a query needs more than stored memory, Daemon enters a multi-round ReAct loop with **20 tools**:
...
**~5,700 tests across 235+ test files** (exact live count in the badge above and `docs/METRICS_SNAPSHOT.md`; run `pytest --collect-only`). Coverage spans every subsystem — prompt-section eval (246), synthesis audit (40), knowledge graph, intent classification, web-search trigger, fact verification, escalation FSM, cross-deduplication, claim tracking, visual memory, and retrieval benchmarks (real embeddings, recall@K + MRR), among others.

<!-- METRICS:BEGIN -->
<!-- Generated by scripts/generate_doc_metrics.py — do not edit by hand. -->
_Snapshot: 2026-07-02 · git `2009d9b`_

- **Python:** 528 files · 185,543 total lines · 140,770 non-blank/non-comment  <sub>(git ls-files '*.py', excl venv/ data/)</sub>
- **Tests:** 5747 collected across 236 test files  <sub>(python -m pytest --collect-only -q; pytest.ini exclusions applied)</sub>
...
tests/                       # 235+ test files, ~5,700 tests
```
...
Central config: `config/config.yaml` (52 sections) → Pydantic v2 validation (`config/schema.py`) → ~300 module-level constants (`config/app_config.py`) with environment-variable overrides. The active model is multi-provider and config-selectable.
