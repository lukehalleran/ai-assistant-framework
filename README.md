...
weights w_i (per-intent overridable):
...
Three generator tiers feed a 7-stage filter; the filter is the product (generation is cheap, *deciding what's evidence-backed and non-obvious* is the value):

- **Tier 0 — Retrieval** (`synthesis_retriever.py`): structural query extraction → FAISS search → adversarial evaluation
- **Tier 1 — Graph Walk** (`graph_walk_generator.py`): hub-dampened, cross-domain-constrained Markov walks over a unified personal + Wikidata graph
- **Tier 2 — Cross-Store** (`synthesis_generator.py`): entity sampling from facts + wiki, LLM bridge articulation
...
+-- agentic/                 # ReAct agentic tool loop (20 tools, one dispatch table)
