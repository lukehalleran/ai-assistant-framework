# memory/storage/chroma_store.py - ID assignment fix
...
class ChromaMemoryStore:
    def add_memory(self, content: str, metadata: dict = None):
        """Add a memory to ChromaDB with proper ID generation"""

        # Generate deterministic ID from content hash
        # This ensures same content gets same ID (deduplication)
        content_hash = hashlib.md5(content.encode()).hexdigest()[:12]
        timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
        memory_id = f"mem_{content_hash}_{timestamp}"

        # Alternative: Use UUID for always-unique IDs
        ...
        # Add to ChromaDB
        self.collection.add(
            ids=[memory_id],  # ChromaDB requires list of IDs
            documents=[content],
            metadatas=[metadata]
        )
    ...
    def batch_add_memories(self, memories: list):
        ...
        for mem in memories:
            ...
            if isinstance(mem, dict):
                content = mem.get('content', mem.get('response', str(mem)))
        ...
        if ids:  # Only add if we have items
            self.collection.add(
                ids=ids,
                documents=documents,
                metadatas=metadatas
