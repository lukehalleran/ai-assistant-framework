...
class FactExtractor:
    ...
    def __init__(self, use_rebel: bool = True, use_regex: bool = True):
        ...
        # e.g., "my cats name is Biscuit", "my car make is Honda"
    ...
    def _extract_with_spacy_rules(self, text: str) -> List[Tuple[str, str, str, float, str]]:
        ...
        for sent in doc.sents:
            ...
            for cop in [t for t in sent if t.dep_ == "cop"]:
                ...
                # Prefer proper-name subjects in span (e.g., “my kitten Biscuit is …”)
                s_candidate = None
                ...
                # Preserve proper-case for PROPN complements (e.g., Skyrim, Biscuit)
            ...
            for name_tok in [t for t in sent if t.lemma_.lower() == "name" and t.pos_ == "NOUN"]:
                ...
                # e.g., "My cat's name is Biscuit" → name --nsubj--> is --attr--> Biscuit
                if not (cop and attr):
                    head = name_tok.head
