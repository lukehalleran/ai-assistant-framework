...
- The pre-push hook's full unit pass is the real test — a batch's targeted run is not (2026-09-20:
  the full pass caught a test that two targeted runs had both missed).
- A subagent's claimed "pre-existing failure" is re-run on the pristine base clone before it is
  believed.
...
run in `hooks/pre-push` and CI; a guard is lowered by a batch, never raised. Import doctrine (the
CLAUDE.md five allowed function-body-import cases) is enforced the same way — its ratchet lives in
`tests/unit/test_import_hygiene_guard.py`, and checking it means running from the clone root as
`env -u PYTHONPATH … python -s` so the scan sees the clone's own code, not the live checkout's.
