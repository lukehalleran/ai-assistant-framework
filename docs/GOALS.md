...
- Entity match via `extract_graph_entities()`: queries mentioning known entities (Biscuit, Sam, etc.) auto-route to agentic memory search
...
- `rank_expansion_candidates()` in `graph_utils.py` — ranks expansion terms by lateral connectivity (non-hub edge count) instead of name length. Real entities (Biscuit=8 edges) now rank above junk phrases ("coffee"=0 edges).
