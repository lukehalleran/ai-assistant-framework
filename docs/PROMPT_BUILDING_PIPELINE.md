...
| graph_context | `get_graph_context()` | 12 sentences (`max_graph_sentences` override; casual_social=0). Traversal uses `hub_barrier=True` — never expanded THROUGH `user`/hubs, `mentioned_alongside` skipped, seed-incident edges first, reached hub edges last [2026-09-03] |
