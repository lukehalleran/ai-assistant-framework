...
| BC-89 | Small-budget structured LLM call on a reasoning-capable model | E | partial |
| BC-90 | Per-route request-shape constraint lives outside the capability registry | B | partial |
| BC-91 | Machinery text treated as content by a downstream consumer | E | partial |
...
- Incidents: 2026-08-31 `PATTERN_ANALYSIS_ENABLED` gated the agentic tool but not the insight facet (AUDIT 08-31 F17, found by two auditors). 2026-09-21 the no-card backstop had no expected-to-act gate while the external-claim correction ten lines above it did; `hooks/pre-push` pass 1 had no Daemon gate while pass 3 did, and changed non-unit tests rode pass 1.
...
- Incidents: 2026-08-31 `_SOURCE_MATERIAL_TRUNC` vs the 6000-char collector (F23); `GOOGLE_CALENDAR_MAX_EVENTS` as fetch limit AND batch cap (F35). 2026-09-19 one env NAME read with two literal defaults: `GATE_COSINE_THRESHOLD` 0.45 in a dead `context_gatherer` constant vs gate_system's live 0.50, and `REFLECTIONS_ON_DEMAND` on in the summarizer vs off in its only caller (pinned by `tests/unit/test_env_default_single_source.py`; 12 more names in the env-read inventory).
...
- Incidents: 2026-08-28 corpus `{query,response}` vs retrieval `{content}` dedup keys never collided; 2026-09-09 `/api/graph` assumed a node list (F09); `get_ids_by_timestamp_range` passed ISO strings to numeric `$gte` — raised on every call since it shipped, `source_doc_ids` never written (F05); 2026-08-18 `add_summary` dict payload.; 2026-09-22 the memory TOP-UP (`core/prompt/builder.py` `_key`) keyed raw `query+response` while `_get_recent_conversations()` returns copies carrying the read-time personal-claim MARKER and BACKFILL recents are raw corpus dicts — the 08-28 `_canonical_turn_key` closure was bypassed by a second key; cross-section dedup removed a 14 K-char paste and the top-up re-added it 3/3 turns (`_canonical_turn_key` itself also split short marked/raw turns). Closed by `_topup_filler`/`_recency_floor_filler` + machinery-stripping key.
...
- Incidents: 2026-08-18 `GATE_MIN_MEMORIES` forced 8 below-threshold memories into "Hey"; 2026-08-29 top-up refilled 1 → 30 ungated; 2026-09-05 floors ran after the budget and restored uncapped records (INDEPENDENT F1); upload bar 0.5 ≈ any text (08-14/27).; 2026-09-22 the survival top-up (`MEMORY_TOPUP_FLOOR`=3) re-admitted the very item cross-section dedup had just removed, three turns running (~1 K tokens each after middle-out).
...
- Incidents: 2026-08-29 verifier never saw the retrieved syllabus; 2026-09-02 planner ran in parallel and reversed speaker/addressee (CLAIM_SUPPORT T02); 2026-09-06 `_CONTEXT_DIGEST_KEYS` omitted `user_uploads` (F3); 2026-09-07 reuse gate took any tool result as "evidence seen" (R3); 2026-09-20 `gui/handlers._run_doc_generation` passed `ctx.user_text` (180 chars) as the document generator's `source_material` while the attachment text existed only in `ctx.merged_input` on the SAME context object — the narrower view failed `DOCUMENT_PROVIDED_MIN_CHARS` and the generator ran RESEARCH mode on the literal request text, which was also taken as the topic/filename (fixed: `_attachment_source_material(ctx)` reads `merged_input` first).
...
- Incidents: 2026-08-05 curated profile writes wiped by the daemon's save; 2026-08-21 the guard itself defeated by a relative-path launch; 2026-08-22 pytest seeded the live floor via `tone_state.json`; 2026-09-01 a lazily-importing test wrote prod `pending_actions.json`; 2026-09-02 48 synthetic facts + 35 edges live from a calibration script; 2026-09-05 `graph_junk_cleanup.py --apply` had no guard. 2026-09-19 sixteen store-writing scripts' `_daemon_running()` fell back to `return False` (or the cmdline heuristic with the known relative-launch hole) when `utils.daemon_guard` could not be imported — fail-open; now fail closed (`tests/unit/test_script_daemon_guard_fail_closed.py`). 2026-09-20 `tests/unit/test_insight_mode_handler.py::test_assessment_kind_runs_assessor` reached the REAL `DocumentGenerator.save_prewritten` (agree-verdict doc save) with no sandboxing and wrote 61 files into the owner's real `documents/` tree over several weeks, plus stray index rows (fixed: conftest autouse `_sandbox_generated_documents` patches `app_config.DOCUMENT_OUTPUT_DIR` to `tmp_path`; pinned by `test_documents_dir_is_sandboxed`). 2026-09-21 `hooks/pre-push` asked `daemon_running()` about the CLONE's root (the live Daemon invisible; the 09-14 hard reset), and the rewrite's first draft probed once, minutes before the 8G pass (referee finding; now re-probed immediately before it). 2026-09-22 review: pre-push contract fixtures inherited DAEMON_LIVE_REPO_ROOT from the owner shell, making isolated-root tests depend on the live process; fixture environments now clear it and allow explicit per-test injection.
...
- Incidents: 2026-08-03 trailing `e`; 08-14 raw 402 streamed three times; 08-22 tag shell; 08-31 3.5-min degenerate loop; 09-03 `[OpenAI unavailable` (50 docs) (RETRO §3.8); 2026-09-20 the same trailing-`e` artifact hit a NEW shape — `DocumentGenerator.classify_deliverable`'s exact-word label compare read kimi-3's `"DERIVATIVEe"` as no label, intermittently mis-routing to the wrong deliverable branch (fixed: prefix-based label match, not just exact-word); same evening, a NEW LLM-output consumer, `compose_from_material`, skipped the shared edge sanitizer entirely — `<|sep|>` leaked into a saved document, hid the H1 from `_extract_title` (title fell back to "SUMMARY"), and printed in the exported `.docx` (fixed: applies the same `ResponseParser.strip_trailing_stream_artifact`/`strip_stream_special_tokens` cleanup `generate()` already had). 2026-09-21 kimi-3 ended a reply `<|close|>think<|sep|><|open|>response<|sep|><|close|>message`; `_EDGE_SPECIAL_TOKEN_RE` stripped only runs of PURE tokens, and this run ends in a bare channel word → displayed and stored (0278d90).
...
- Incidents: 2026-08-28 review gate's only production "fix" was a silent swap (→ log-only); 2026-08-31 agentic debug record built before grounding ran, `grounding_corrected` set before integration (F9/F24); 2026-09-08 no-round line stripper mutated display only — fences vanished from the bubble, history intact (F1/P0).; 2026-09-22 the personal-claim background audit ran over the DELIVERED text including the action-guard suffix (`gui/handlers.py` suffix appended, then the audit received `display_output`): receipt 6 candidates / 3 insufficient on a reply whose only unsupported material was the guard's own notice; the stored reply carried the marker. Closed: audits and grounding read the pre-suffix reply; digests hash machinery-free text. 2026-09-22 review: sequential grounding and personal-claim correction stages both read a frozen original answer, allowing the later revision to discard the earlier one; preserve the latest revised body separately from delivery notices.
...
- Incidents: 2026-08-29 correct semester flagged 0.9; Saturday→Sunday invented; 2026-09-01 correct Sep-13 rewritten to Sep-6 at 1.00 against its own source; PSA spliced over the user's own figure (09-02). 42 fires → 25 shipped corrections, ≥9 false, 0 documented true.; 2026-09-22 DETERMINISTIC sibling: the calendar STATE-claim backstop (`gui/handlers.py`) corrected "sleeping til 10 when you had a noon appointment" — the user's own report in the same message — because it checked the clause only against gathered Google events; shipped `> ⚠️ I don't see that on your calendar — nothing was created`, stored, re-rendered next turn. Closed: `_calendar_claim_user_corroborated` (the clause's own calendar noun found in a user-authored span = evidence). 2026-09-22 review: sharing a generic calendar noun admitted a request or negated mention as corroboration for a fabricated calendar-state claim; evidence must match the asserted proposition, not just its vocabulary.
...
- Incidents: 2026-09-03 cat called a dog (edge + label + planner aligned); "birthday celebration" from "turned 2"; 2026-09-04 "withdrawn from the semester" vs a one-course drop; 2026-09-05 "six consecutive days" copied across days.; 2026-09-22 the STM paraphrase "Confirm whether to attend…" refined a first-person status update to `factual_recall@0.60` via `_STM_KEYWORDS["confirm"]`, injecting the FACTUAL RECALL style block on "I looked yesterday just two but I will check again…". Closed: recall targets skipped on `is_self_report`.
...
- Incidents: 2026-09-02 "six days" vs "two weeks" flattened in the narrative (CLAIM_SUPPORT T04); 2026-09-06 completed plans still shown pending, streak warnings quoting a discarded claim (AUDIT 09-06).; 2026-09-22 (a) the same STM refinement TAUGHT the sentence as a factual_recall exemplar (n=40) — a label derived from an LLM paraphrase, not the user's words, entered the learner (closed: recall-family teaching needs the cue in the user's text); (b) "fair's Sunday" for Monday 2026-09-28 — the model copied Daemon's own 20:41 weekday error from [RECENT CONVERSATION]; [GOOGLE CALENDAR] rendered no weekday (closed: weekday in every calendar line).
...
- Incidents: "for a bit" (08-02); `has_dad="dad is picking up at 5:30"` (09-03); `works_on="this assistant"` (09-05); `gym_schedule` from a calendar request (09-08).; 2026-09-22 graph node `again` (created 2026-08-10) passed `is_junk_entity` (also `lol`), was matched from the query word and seeded graph context + the ranker boost ("again is two weeks ago"). Contained: untyped lowercase stopword candidates are filtered using spaCy's maintained lexicon; name evidence is preserved rather than assuming stopwords can never be names. 2026-09-22 review: a stopword lexicon also contains possible names such as May; graph filtering now preserves named-entity type and original casing, and subject/object/extraction/expansion paths have counterexamples (DM-39). Untyped lowercase homographs remain ambiguous.
...
- Incidents: 2026-09-15 a committed test fixture quoted a LIVE reply verbatim, medication names included — the pre-commit privacy hook (`config/privacy_terms.local.txt`) blocked the commit; fixture genericized (the shape, never the owner's vocabulary); `_SELF_MODEL_NOUNS`, `_PERSONAL_MARKER_RE`, `HEAVY_KEYWORDS` conflating personal and professional vocabulary (GENERALIZATION 09-01 P1); 2026-09-12 the 2026-08-27 academic-logistics cue list (`institution_resolver._ACADEMIC_CUE_RE`) was built from one student's drop-date turn and treated cross-domain words as school vocabulary — withdrawal (medication, troops), registration (voters), enrollment (Medicare), transcript (a court hearing) — so a medical, civic or legal query carried the owner's school into the search provider. 2026-09-21 `utils/web_search_trigger._PERSONAL_ROUTINE_NOUN_RE` (the 2026-09-10 personal-routine fix) listed the owner's medication by name beside melatonin/caffeine; found by a case-insensitive scan of master with the private term list; the noun now comes from `personal_vocabulary.category_tokens.health` (`_personal_health_terms()`, read live) — remedy pattern 1 EXTERNALIZE (7b77484). 2026-09-22 review: new graph regression examples included owner-private literals, while calendar fixtures copied a private transcript; public fixtures were replaced by synthetic equivalents retaining the failure structure.
...
- Incidents: 2026-08-28 vacuous double-filtering file; 2026-09-09 11 generator bodies (27 undetected corrupt cases), 31 more across 12 files, two fossils calling nonexistent methods, `debug_memory_state` crash hidden as a skip. 2026-09-19 an unconfigured `AsyncMock().return_value` is another AsyncMock, so production's plain `.sort()` made an un-awaited coroutine in `test_prompt_builder_self_report_trim` (each touched test mutated to prove it fails).
...
- Incidents: the five Critical-Rule incidents (inverted oracle, L2-as-cosine, mocked `.intent_type`, monotone AUC, `distance` key); `test_prompt_timeout` rebuilt the gather loop (T02); getsource-only coverage of a 230-line branch (F12); hardcoded date vs live clock (F4); shape-only assertions (T12/T13); 2026-09-14 `scripts/safe_cmd.sh`'s unlock path and its BLOCKED message have been unreachable for protected paths since the script's first commit (2026-05-21) — `tests/unit/test_destructive_op_guard.py` exercises only the Python helper `utils/destructive_op_guard.unlock_allowed`, never the deployed shell wrapper, so nothing went red (the defect itself is the `set -e` singleton below). 2026-09-19 `scripts/safe_cmd.sh`: tests covered only the Python classifier, never the deployed wrapper, whose `set -e` killed it before the BLOCKED message and the unlock check — the documented unlock had never worked since 2026-05-21 (`tests/unit/test_safe_cmd_wrapper.py` drives the script); `test_process_signals` pinned the shutdown latch by source text only. 2026-09-22 review: the two-boot smoke test was excluded by the fast CI marker filter and could skip when its cache was absent. The required test job now provisions models, explicitly runs smoke and rejects empty/skipped/wrong-case receipts; it proves isolated runtime persistence, not installation or Windows packaging.
...
- Incidents: Backblaze dead 46 days; daily-notes timer failing 7.5 months (452 tracebacks, exit 0); wiki FAISS index unmounted; People API 403; OpenRouter credits out mid-session (RETRO §3.15). 2026-09-19 `configure_logging()` renamed a LIVE instance's `daemon_debug.log` on every refused second launch (rotation at import, single-instance lock ~700 lines later), leaving the running Daemon appending to an archive name; `get_codebase_changes` ran four blocking git `subprocess.run` calls on the event loop per prompt build; a swallowing `except` turned an AttributeError into a silent empty wiki result during the same batch. 2026-09-21 the private-term scan had not run for ANY commit made in a lane clone since 2026-09-13 (PRs #8–#27): `git clone` copies neither `.git/hooks` nor the gitignored list; seven listed-term occurrences reached the public master. Closure: fail-closed hook, commit runners refuse without both hooks + the list, pre-push refuses without an installed pre-commit hook.
...
- Status: partial — sync-notes and the curation page are closed; any new long-running route needs the same retained-state or lost-response treatment (no static check).
...
- Incidents: 2026-09-14 10:05:36 the sync route's bare `ObsidianManager()` (`api/routes/system.py`) lazily built a second store inside the running Daemon and logged a fresh `[Chroma] Embedder BAAI/bge-small-en-v1.5 on device=cuda` load, although the constructor accepts the live store (`ObsidianManager(chroma_store=...)`) and no caller passes it. 2026-09-16: the same route built a second store again on the recurrence (see BC-80); `gui/launch.py` notes button had the same bare constructor. 2026-09-19 CONFIRMED live: `core/prompt/context_gatherer.py` built `ObsidianManager()`, `ReferenceDocsManager()` and `WebSearchManager()`→`WebSearchCache()` with no store — `[Chroma] Embedder … on device=cuda` logged four times per process; fixed by `_live_chroma_store()` injection (`tests/unit/test_gatherer_shares_live_store.py` drives the real lazy properties; every earlier test set the private attribute to a mock).
...
- Status: partial — notes-sync (09-16) and the context gatherer's three managers (09-19) are closed; `HybridRetriever` and `GitMemoryLoader` fallbacks verified unreachable from the serving process; open candidate `core/prompt/proposal_filter.py` builds an uncached MiniLM `SentenceTransformer` per call. No static check: a manager's own lazy fallback stays legitimate for CLI/scripts.
...
- Incidents: 2026-09-14 (caught before landing) an uncommitted launch-auth batch rejected every non-loopback `Host` with 400 "invalid host", while the owner's deployment binds the API to a non-loopback remote-access address through the `config.local.yaml` `api.host` override — after landing, every request including `GET /` would have failed. The batch had scoped the non-loopback bind out as a known limitation without checking the live override; a host allowance was requested before landing. 2026-09-20 `DocumentGenerator._index_path()`'s `relative_to(repo_root)` assumed `document.output_dir` is always under the repo root — true only under the committed default — so a test sandbox override (or a `config.local.yaml` override pointing generated-document output outside the repo) raised `ValueError` AFTER the document file was already written, silently orphaning it from the index (fixed: absolute-path fallback when the relative computation fails). 2026-09-21 `reasoning:{enabled:false}` (the 09-06 agentic fix, validated on kimi-3/deepseek) is refused with HTTP 400 by claude-fable-5, registered since June; nobody had run an agentic turn on it.
...
## M. New mechanisms (2026-09-20 attachment doc-generation session)

One mechanism from a 2026-09-20 evening session (fixing an attachment-sourced
document-generation turn — `docs/PLAN_20260912_session_defects.md`-adjacent,
see `CLAUDE_CHANGELOG.md` "2026-09-20 evening") did not match any of the
existing classes closely enough to file as an incident. It takes BC-89: BC-88
had already been claimed by the 2026-09-20 env-read lane ("setting read straight
from the environment", `tests/unit/test_env_read_guard.py`), merged as PR #33. The other candidates from the same session (a downstream
consumer reading a narrower field than the one gathered, a deliverable
vocabulary with no expressible second member, a stream artifact reaching a
new consumer shape, an assumption validated only under the default
configuration) matched BC-25, BC-15, BC-44 and BC-82 respectively and are
filed as incidents there instead of stretched into new entries.

### BC-89 Small-budget structured LLM call on a reasoning-capable model
- Mechanism: a call expecting a single word/letter/short label (a classifier
  verdict, a pairwise vote, a category name) sets a small `max_tokens` without
  disabling the model's reasoning channel. On a reasoning-capable model —
  which model is active is a user setting (`config.yaml` `models.active`,
  changed frequently) — the reasoning channel consumes the ENTIRE budget
  before any visible token is emitted, so the call returns an empty string.
  Callers treat empty/unclear as a legitimate negative and fail safe to a
  hardcoded default branch, so the failure is silent: no exception, no retry,
  just the wrong path taken with high confidence. Distinct from BC-14 (a
  `TypeError` from a kwarg mismatch — this call SUCCEEDS, it just returns
  nothing useful) and from BC-46 (a structural input overriding a prompt
  instruction — here nothing overrides the instruction, the budget is simply
  spent before the instruction can be followed).
- Incidents: 2026-09-06 agentic decision calls sent NO reasoning key at all
  (kimi-k3 reasoned ~45s/round) — fixed via
  `generate_once(disable_reasoning=True)`; 2026-09-20
  `DocumentGenerator.classify_deliverable` was called with `max_tokens=8` and
  reasoning ON — kimi-k3 spent the whole budget in the reasoning channel
  (`[PromptCache] … completion=8`), returned empty, and the documented
  never-a-guess fail-safe silently chose "analysis" over the correct
  "derivative" branch (a resume-rewrite request was answered with a cited
  report ABOUT the resume) — fixed via `disable_reasoning=True` +
  `max_tokens=64`.
- Find: DM-36 — `tests/unit/test_small_budget_llm_call_guard.py` (self-contained
  AST scan for a `Call` whose attribute/name is `generate_once` carrying a
  literal `max_tokens` keyword <= 64 with no `disable_reasoning=True`
  keyword); manually, grep `generate_once(` call sites whose prompt asks for
  "exactly one word/letter" and check `max_tokens` and `disable_reasoning`
  together, not just one of the two.
- Closure: per-incident (`disable_reasoning=True` + headroom at each found
  site); `tests/unit/test_small_budget_llm_call_guard.py` is a NEW ceiling
  ratchet (CM-02), not yet a repo-wide fix — its first run (2026-09-20) found
  5 sites total, 4 of them NEVER audited for this class before now:
  `core/prompt/proposal_filter.py` (pairwise proposal ranking, `max_tokens=4`),
  `gui/wizard.py` (API-key smoke test, `max_tokens=10`),
  `memory/fact_verification.py` (A/B/C adjudication, `max_tokens=4`),
  `memory/user_profile_schema.py` (category classification, `max_tokens=10`),
  and one sibling left inside the SAME file the incident was fixed in —
  `knowledge/document_generator.py`'s `_refine_topic` (`max_tokens=30`, no
  `disable_reasoning`) — a BC-58 sibling-coverage gap on day one of the guard.
- Status: partial — the two named incidents are fixed at their own call
  sites; the other four sites the guard's first run found have not been
  triaged (some may pin non-reasoning models via `model_name=` and be
  low-risk; none have been checked).

## N. New mechanisms (2026-09-21 model-roster session)

Registering three OpenRouter models (GPT-6 Astra, Claude Fable 5.1, DeepSeek V4.1 Flash) surfaced one
mechanism no existing class named. Every other finding of that day is filed as an incident on its class
(BC-04, BC-13, BC-37/38, BC-44, BC-47, BC-58, BC-59, BC-61, BC-69, BC-71, BC-76, BC-82, BC-83).

### BC-90 Per-route request-shape constraint lives outside the capability registry
- Mechanism: a provider route REFUSES (HTTP 400) a request shape that every other registered route accepts — an off-switch
  it does not honour, a selector it does not implement, a fixed parameter value. The capability registry records what a
  model CAN do (reasoning/vision/tools/caching) but not what its endpoint will not ACCEPT, so the shape is built from a
  literal at each send site, written against whichever model was active when the path was last debugged. Which model is
  active is a user setting that changes often, so the refusal appears only when the owner selects that model and only on the
  paths that send the shape (a recovery retry, a forced tool round) — it reads as a flaky provider. The public model catalog
  is not evidence either way: it listed `tool_choice` and `reasoning` as supported for a route that refused both shapes.
  Distinct from BC-10 (a registered entry missing from a parallel list: here no list has a slot for the fact), from BC-82
  (validated only under the committed default: that is how it stays hidden, not what it is) and from BC-89 (the call
  succeeds and returns nothing: here it is refused).
- Incidents: 2026-07-30 moonshotai/kimi-k3 400s on any top_p other than 0.95 (closed with the first request-shape row key,
  `forced_top_p`); 2026-09-21 `reasoning: {"enabled": false}` refused by anthropic/claude-fable-5 — registered since
  2026-06-09, so every `disable_reasoning=True` call on it (agentic tool rounds since 09-06, reasoning-only recovery, the
  BC-89 closures) had been failing — and by the two routes being added, anthropic/claude-fable-5.1 and openai/gpt-6-astra;
  same day anthropic/claude-fable-5.1 refused the named `tool_choice` the agentic forced-action round sends. A 09-13 patch
  for the same three models, derived from the catalog alone, had the tool_choice fact right for the wrong reason, missed
  claude-fable-5, and proposed an error string returned as content for the forced round.
- Find: DM-37 (paid, opt-in) `scripts/probe_model_request_shapes.py --run --all` — each row's declared constraints against
  the live route, plus the deployed substitute shape; then `rg -n '"reasoning"\]? *=|tool_choice|top_p' models/ core/` for a
  request object built from a literal outside the chokepoints.
- Closure: CM-01 — request-shape keys on the MODEL_CAPABILITIES row (`forced_top_p`, `reasoning_mandatory`,
  `forced_tool_choice`), one builder per shape (`resolve_top_p`, `reasoning_request_config`, `resolve_tool_choice`), all send
  sites call the builder; `tests/unit/test_model_request_shapes.py` drives the deployed generate paths with a capturing
  transport; registry parity validates the keys.
- Status: partial — the probe is manual and costs credits; adding a model should include one `--run --models <new>`; the gpt-5.x/GLM rows declare reasoning=False although the routes reason, so a small-budget call on them cannot send any off-switch (BC-89 exposure, unprobed).

### BC-91 Machinery text treated as content by a downstream consumer
- Mechanism: text the system itself appends to an assistant reply at delivery or read time (guard notices `> ⚠️ …`, the read-time personal-claim MARKER, `[unverified action claim]`) is later consumed by a component that assumes it is reading the reply — a dedup key, an audit, a digest, a planner digest, a render — so the machinery changes identity, verdicts or context. Distinct from BC-75 (the model's OWN narration laundered into a store) and BC-45 (display ≠ stored): here display == stored, and the consumer is wrong about what the bytes ARE.
- Incidents: 2026-09-22 top-up dedup key mismatch on the read-time marker (3 duplicate 14 K-char pastes); 2026-09-22 personal-claim audit over the calendar notice (3 "insufficient" from the guard's own text; marker applied to the stored reply); 2026-09-22 the notice + marker carried into [LAST EXCHANGE FOR CONTEXT] and the planner's context; 2026-09-11 (B12/B13) the reverse direction — an UNMARKED stored claim read as settled fact — is what created the markers in the first place. 2026-09-22 review: adjacent authored blockquotes prevented notice stripping, while broad warning-tail stripping could discard authored text. Registered delivery notices and suffix-boundary regressions constrain removal.
- Find: DM-38 (gate) — `python scripts/check_bug_classes.py scan --root .`; for a consumer OUTSIDE its declared sites: `rg -n '\.get\("(query|response|content)"' core/prompt/ gui/ | grep -v _canonical_turn_key` (raw identity/digest of a stored turn), any audit/digest fed a name that was rebound `… + <suffix>`, any render of a stored assistant turn without `strip_delivery_notices`, any `"> ⚠️"` literal outside `utils/read_time_markers.py` (`rg -n '> ⚠️' --glob '!tests/**' --glob '!utils/read_time_markers.py'`).
- Closure: `utils/read_time_markers.py` (single definition of every marker AND every notice family: the `NOTICE_*` openings are the registry AND the strings the emitters compose through `delivery_notice(NOTICE_*, detail)` — one copy, no drift; `strip_read_time_markers`, `strip_delivery_notices` (suffix-only: registered families at the tail, an authored blockquote before them survives), `strip_machinery`); all seven emitters (`core/action_claim_guard.py`, `utils/web_evidence_receipt.py`, `gui/handlers.py`) route through the factory; the two delivery sites share `_apply_delivery_revisions` (grounding → personal-claim on the revised body, guard suffix reattached once); `_canonical_turn_key` and both top-up/floor fillers strip machinery; receipts and the read-time annotator hash machinery-free text; renders strip delivery notices before re-annotating. Tests: `tests/unit/test_read_time_markers.py`, `test_sep22_topup_dedup_parity.py`, `test_sep22_formatter_machinery_strip.py`, `test_sep22_calendar_backstop_user_words.py`.
- Status: partial — DM-38 gates the declared sites and the emitter registry (landed 2026-09-22, policy 2026-09-22.1, 326-case harness), behavioral tests cover the known consumers; it is not whole-program taint analysis, so a NEW consumer shape outside the declared sites still needs the BC-58 sibling review. Structured notice storage (notice as metadata, not bytes in the reply) and full G12 closure remain open.
...
| DM-36 | `generate_once()` calls with a literal `max_tokens` <= 64 and no `disable_reasoning=True` keyword; self-contained AST scan, ceiling ratchet (CM-02) | `tests/unit/test_small_budget_llm_call_guard.py` | BC-89 |
| DM-37 | Paid, opt-in: each registered model's DECLARED request-shape constraints against the live route (off-switch, forced tool_choice) plus the deployed substitute shape; dry run by default | `scripts/probe_model_request_shapes.py --run --models <alias…>` | BC-90 |
| DM-38 | Machinery-text consumers + emitter registry, four stdlib AST rules, gate: (1) both memory fillers in `core/prompt/builder.py` canonicalize every candidate via `_canonical_turn_key`; (2) each delivery site in `gui/handlers.py` hands a pre-suffix snapshot (no `*_suffix`/notice rebinding on the chain) to ONE ordered `_apply_delivery_revisions` pipeline, never calls a checker directly, and inside it the personal-claim check reads the grounding-REVISED body; (3) the three assistant-turn render sites in `core/prompt/formatter.py` rebind through `strip_delivery_notices`; (4) no `"> ⚠️"` literal outside `utils/read_time_markers.py` and every `delivery_notice(...)` opening is a registered `NOTICE_*` constant. Bounded to those sites — not taint analysis; 13 red/green mutation controls | `scripts/check_bug_classes.py scan` + `tests/bug_class_guards/test_scanners.py::TestDM38*`; behavioral mirrors in `tests/unit/test_read_time_markers.py` (emitter parity), `test_sep22_topup_dedup_parity.py`, `test_sep22_delivery_revision_sequence.py`, `test_sep22_formatter_machinery_strip.py` | BC-91, BC-20, BC-45 |
| DM-39 | Deployed graph lexical-junk gate, named-entity homograph controls and ingestion/extraction/expansion parity; required English lexicon, no silent dependency skip | `tests/unit/test_sep22_graph_function_words.py` (ordinary CI fast/unit lane, not the stdlib structural scanner) | BC-55, BC-58 |
...
Together they are a scoped structural lane. Each scan computes the current
catalog/scanner coverage and lists every class without a registered scanner;
ordinary behavioral unit guards are reported separately from that structural scope. A green scan is not a behavioral guarantee for any class.
...
A 2026-09-22 turn audit (lane GF) added BC-91 (machinery text treated as
content) with three same-day incidents plus a BC-55 function-word incident;
the same-day adversarial review (Codex, then Fable integration) landed DM-38
as a gated scanner (eighth in the policy, contract v2 with the emitter
registry) and DM-39 as an ordinary unit-lane guard, and found that the first
replacement of the calendar corroboration helper had silently regressed the
live shape it was written for (the synthetic live-shaped case is now pinned).
...
- Startup side effect runs before the ownership check: log rotation at import vs the single-instance lock (09-19, fixed via `instance_lock_held_by_other`).
- Non-raw string with a bare backslash (`\/`, `\ `) — DeprecationWarning today, SyntaxError later (09-19, two sites fixed; guarded by `tests/unit/test_no_invalid_escape_sequences.py`).
- Deprecated `asyncio.get_event_loop()` used inside a coroutine to fetch the running loop (09-19, six sites moved to `get_running_loop()`; five remain in accepted-debt files).
