...
| 9 | **`hooks/pre-push` memory cap kills non-unit pushes** (found 2026-09-10) — the changed-files selection runs in ONE pytest process under `MemoryMax=6G`; a push range touching non-unit test files (the T01 repair: 10 of them) peaks at 5.97 GB and is SIGKILLed ("Killed … BLOCKED — tests"), no test failed; the same selection passes under the §3-documented 8G cap (7.1 GB peak, `~/daemon_checkpoints/prepush_nonunit_8G_20260910.txt`); the T14 mapping also pulls in the CLIP-loading `test_visual_memory_pipeline.py` (harmless: unit half peaks 1.3 GB); the hook's `PREPUSH_FULL` path carried 4 `--ignore`s from the retired CI list — removed 2026-09-10 to match CI | `hooks/pre-push` (runner line + T14 loop) | raise the cap to 8G to match `docs/DEVELOPMENT_WORKFLOW.md` §3; skip ignore-listed files in the mapping; optional: split unit / non-unit into two processes so the non-unit half only runs with the daemon down. Docs already corrected (TEST_LANES §4, WORKFLOW §3a.6, README) |
| 10 | **Docker path is stale** (found 2026-09-10 doc audit) — `docker-compose.yml` maps `7860:7860`, sets only `GRADIO_*` env, and healthchecks `:7860/health` (Dockerfile too), but `CMD ["gui"]` takes main.py's default FastAPI branch bound to `API_HOST:API_PORT` = 127.0.0.1:8000, so nothing listens on the mapped port and the healthcheck fails | `docker-compose.yml`, `Dockerfile`, `docker-entrypoint.sh` | bind the API to 0.0.0.0:8000 in the container (env override), remap ports + healthcheck to 8000 (confirm the FastAPI app serves `/health`), or run `--legacy-gui` explicitly; README marked OUT OF DATE meanwhile |
| 11 | **Web search silently absent on news shares + confabulated "search came back empty"** (found 2026-09-10 from the owner's 4-turn dump) — (a) `The president says he will pay everyone 5000 if his party wins midterm. Oh boy` (13:46) never reached Tavily: `requires_fresh_public_evidence` needs temporal AND question AND public-event cues and a statement-shaped share has neither of the first two (returns False on the exact text; `presidents?` matches, `says` is in neither event regex); the heuristic scored 0.0/no keywords, and on that combination `analyze_query_with_llm` SKIPS the LLM trigger ("Skipping LLM: heuristic confident no-search"), so nothing ever judged it; the gate's Tier-1 arm uses the same rule. (b) Yesterday 19:47 `Uhm. Please investigate thank you` (after a pasted IRGC carrier claim): same skip (no context-dependence cue, `investigate` is not a search keyword), no tool loop ran, yet the reply said "my web search came back empty this turn" — the model read `[ACTIVE FEATURES] web_search=ON(0)`; `formatter._count` renders `(0)` for an empty result list whether a search ran and found nothing or never ran, and every turn in the dump shows `ON(0)`. (c) Nothing caught either: `turn_records.jsonl` carries NO web-trigger fields (decision/source/reason/results/error) and `tests/unit/test_public_news_verification.py` covers only dated questions and request wrappers, never a bare news-share statement | `utils/web_search_trigger.py` (`requires_fresh_public_evidence`, the conf=0.0 skip in `analyze_query_with_llm`), `core/agentic/gate.py:848`, `core/prompt/formatter.py` (`_count`/`web_search=`), `core/orchestrator._hook_turn_telemetry` | (1) a public-actor STATEMENT arm: strong public-event term + reported-speech/announcement verb (says/said/claims/promises/vows/threatens/will pay…) + no first/second-person pronoun in the remainder → at minimum CONSULT the LLM trigger instead of the conf=0.0 skip (keep the rule conservative; the LLM decides); an explicit `investigate|verify|fact-check` imperative with prior-turn context counts as a search request; (2) render `web_search=ON` as `ON(not triggered)` / `ON(0 results)` / `ON(n)` so the model cannot narrate a search that never ran, plus a prompt line: never claim a search happened unless [WEB SEARCH RESULTS] is present; (3) telemetry: `web_trigger_should_search/source/reason/confidence`, `web_results_n`, `web_error` in every turn record + `latency_rollup`-style read-only counter; (4) regression tests with the two live texts against the deployed functions |
