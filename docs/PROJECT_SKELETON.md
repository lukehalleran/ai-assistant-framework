...
- `memory/pattern_engine.py` - Deterministic pattern aggregation (counts/streaks/gaps/trend by dimension × time bucket) over corpus/telemetry/profile; 5 dimensions; denominator doctrine; on-demand only — see docs/PATTERN_ANALYSIS.md; also hosts LongitudinalEvidenceSpec + run_longitudinal_scan (phased + co_occurrence bucket counting for the deliberation stack) **[NEW 2026-08-29]**
- `core/insight/temporal.py` - Insight-mode pattern_temporal stage: engine-first orchestration + real-exemplar EvidenceItems; mood/music overlays **[NEW 2026-08-29]**
- `core/insight/deliberation.py` - Frozen evidence contracts: fast-model-routed LLM planner + validate_and_freeze (fail-closed: claim-graph cycles, phase overlap, missing source queries) + compact recovery + structural fallback; weekly buckets from explicit/relative windows; series-from-conjunction backstop; deterministic closed-set source restriction **[NEW 2026-08-31]**
- `core/insight/coordinator.py` - LongitudinalDeliberationCoordinator: channel adapters (per-channel concurrency caps) → date-range retrieval arm (window-aware adapters) → longitudinal scan → fail-honest claim assessment (citation floor, dependency propagation, empty-internal-channel demotion) → manifest with reproducibility fingerprints **[NEW 2026-08-31]**
- `knowledge/pubmed_search.py` - PubMed E-utilities: deterministic boolean broadening ladder, planner-emitted concept synonyms, anchor-concept relevance floor, framing-vocabulary stopwords, reported-speech frame strip **[NEW 2026-08-30]**
- `knowledge/research_search.py` - arXiv + StackExchange search rows (epoch-date coercion) **[NEW 2026-08-30]**
- `utils/page_extract.py` - Layered page-text extraction: visible text (bs4) → embedded-JSON salvage (__NEXT_DATA__/turbo-stream/ld+json — ChatGPT share links) backing fetch_url's local-first layer **[NEW 2026-08-29]**
- `scripts/force_reindex_obsidian.py` - Full vault re-embed (daemon-guarded); pairs with obsidian_manager._infer_note_date content-date ladder **[NEW 2026-08-31]**
...
Luke has brother Drew
Drew lives in Portland
...
...
**core/agentic/protocols.py** - Protocol detection and parsing **[ENHANCED 2026-06-08]**
- `detect_protocol(model_name)` → `SearchProtocol` (native for gpt/claude, XML for local)
- `NativeToolsHandler`: Parses OpenAI/Anthropic tool_calls from response. GitHub tool definitions gated by `github_available` param. Empty args for git_stats/github/search_memory default to original query. Calendar-specific params (summary, start_time, end_time, time_zone, calendar_id, location, description) forwarded for `calendar_create_event` actions. **Text-leak recovery (Pattern 4) [2026-06-08]**: when the API returns no `tool_calls` (common with OpenRouter-proxied DeepSeek emitting the call as plain content), delegates the text to `XMLMarkerHandler` and recovers actionable markers so the raw XML doesn't leak into the answer.
- `XMLMarkerHandler`: Parses `<search>query</search>`, `<fetch_url url="...">`, and `<done/>` from local model output. XML alias patterns: `<web_search>`/`<web_search query="...">` → `<search>`, `<search_memory query="...">` → `<memory>`. Nested XML: `MEMORY_NESTED_PATTERN` for DeepSeek-style `<search_memory><query>X</query></search_memory>`. **Nested file/doc/url tool forms [2026-06-08]**: `FILE_READ_NESTED_PATTERN` / `FILE_GREP_NESTED_PATTERN` / `FILE_LIST_NESTED_PATTERN` / `FETCH_URL_NESTED_PATTERN` / `GET_FULL_DOCUMENT_NESTED_PATTERN` parse the child-tag form `<file_read><path>X</path></file_read>` (+ bare-content path form), matching only the attribute-less opening tag so they never double-parse the attribute form. Calendar-specific param forwarding for XML protocol actions. Helpers: `_strip_xml_tags()`, `_extract_nested_tag()`.
...
class GraphEdge(BaseModel):
    ...
    # to_natural_language() → "Luke has brother Drew"
```
...
- Example: "tell me about my brother" → "tell me about my brother Biscuit Sam Mom"
...
A:
Biscuit is your black cat [MEM_SEMANTIC_4]...
