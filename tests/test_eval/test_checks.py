...
class TestThinkingLeak:
    def test_clean_response(self):
        r = check_thinking_leak("Your brother's name is Drew.")
        assert r.passed is True

    def test_thinking_tags(self):
        r = check_thinking_leak("<thinking>Let me check</thinking>Your brother is Drew.")
        assert r.passed is False
    ...
    def test_single_hit_ok(self):
        ...
        r = check_thinking_leak("I should mention that Biscuit is doing well.")
        assert r.passed is True
...
class TestProfileGrounding:
    def test_grounded_name(self):
        ...
        response = "Your brother is named Drew."
        r = check_profile_grounding(response, prompt)
        assert r.passed is True
        assert "Drew" in r.details["grounded"]
...
class TestExtractNames:
    def test_mid_sentence_names(self):
        names = _extract_names_from_response(
            "your brother Drew called and your friend Sam stopped by"
        )
...
class TestFiller:
    def test_clean_response(self):
        r = check_filler("Your brother's name is Drew.")
        assert r.passed is True

    def test_filler_at_end(self):
        r = check_filler(
            "Your brother is Drew. Feel free to ask if you have any other questions!"
        )
        assert r.passed is False
    ...
    def test_no_filler(self):
        r = check_filler(
            "Biscuit is a black cat with golden eyes. He's been bullying "
...
class TestRunAllChecks:
    def test_returns_all_checks(self):
        checks = run_all_checks(
            response_text="Your brother is Drew.",
