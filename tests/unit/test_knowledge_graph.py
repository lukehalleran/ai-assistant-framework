...
class TestGraphMemoryAlias:
    ...
    def test_register_alias(self, graph):
        node = GraphNode(entity_id="biscuit", display_name="Biscuit")
        graph.add_entity(node)
        graph.register_alias("the cat", "biscuit")
        assert graph.resolve_entity("the cat") == "biscuit"

    def test_register_alias_stored_on_node(self, graph):
        node = GraphNode(entity_id="biscuit", display_name="Biscuit")
        graph.add_entity(node)
        graph.register_alias("lil guy", "biscuit")
        entity = graph.get_entity("biscuit")
...
class TestEntityResolver:
    ...
    def test_learn_alias(self, graph, resolver):
        graph.add_entity(GraphNode(entity_id="biscuit", display_name="Biscuit", entity_type="pet"))
        resolver.learn_alias("the cat", "biscuit")
        assert resolver.resolve("the cat") == "biscuit"

    def test_learn_possessive_alias(self, graph, resolver):
        graph.add_entity(GraphNode(entity_id="biscuit", display_name="Biscuit"))
        resolver.learn_alias("biscuit", "biscuit", context="I love my cat Biscuit")
        assert resolver.resolve("my cat") == "biscuit"
...
class TestPossessiveAliases:
    def test_extract_my_cat(self):
        aliases = extract_possessive_aliases("I love my cat Biscuit")
...
class TestIngestFactToGraph:
    ...
    def test_ingest_user_subject_type(self, graph, resolver):
        ...
        with patch("memory.memory_storage._get_graph_enabled", return_value=True), \
             patch("config.app_config.KNOWLEDGE_GRAPH_MIN_CONFIDENCE", 0.5):
            storage._ingest_fact_to_graph(
                subj="user", rel="owns", obj="Biscuit",
