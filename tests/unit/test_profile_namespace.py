...
class TestAddFactCanonicalDedup:
    ...
    def test_pet_alias_dedup(self):
        """Adding 'pet=biscuit' after 'pet_name=biscuit' should confirm, not duplicate."""
        profile = self._make_profile()
        profile.add_fact("pet_name", "biscuit", confidence=0.9)
        profile.add_fact("pet", "biscuit", confidence=0.8)  # alias
