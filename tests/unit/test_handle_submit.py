...
class TestAgenticMemory:
    ...
    async def test_agentic_memory(self):
        orch = _make_orchestrator(
            agentic_enabled=True,
            agentic_items=["Your brother's name is Sam, as you mentioned previously."],
        )
...
class TestAgenticEntityRecall:
    ...
    async def test_entity_recall(self):
        orch = _make_orchestrator(
            agentic_enabled=True,
            agentic_items=["Biscuit is your cat, a senior feline you care for deeply."],
        )
        ...
        with patch("memory.graph_utils.extract_graph_entities", return_value={"biscuit"}):
            results = await _run_submit("what do you know about Biscuit?", orch)
