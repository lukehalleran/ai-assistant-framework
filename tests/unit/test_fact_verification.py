...
class TestVerificationFlow:
    ...
    async def test_entity_scope_low_confidence_rejects(self):
        ...
        existing = [{
            "id": "f1", "content": "biscuit | breed | golden retriever",
            "metadata": {"subject": "biscuit", "relation": "breed",
                         "object": "golden retriever", "confidence": 0.8},
        }]
        v = self._make_verifier(existing_facts=existing)
        r = await v.verify(
            subject="biscuit", predicate="breed", object_val="labrador",
            fact_text="biscuit | breed | labrador",
