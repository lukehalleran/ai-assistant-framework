...
the user corrects an entity-level assumption (e.g., "Biscuit did not die").
"""
...
def biscuit_event():
    return EntityCorrectionEvent(
        ...
        correction_text="biscuit did not die hes still here",
        confidence=0.90,
    )
...
class TestEntityResolutionCascade:
    ...
    def test_crisis_summary_gets_resolution_note(self, orchestrator, mock_chroma_store, biscuit_event):
        """Summary with entity + crisis keyword should get resolution_note."""
        mock_chroma_store.query_collection.return_value = [
            _make_doc("s1", "Biscuit was in the ICU after eating Dexivar"),
        ]

        orchestrator._cascade_entity_resolution([biscuit_event])
        ...
        assert "Biscuit" in md_updates["resolution_note"]
        assert "alive" in md_updates["resolution_note"]
        assert md_updates["staleness_ratio"] == 0.65

    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_non_crisis_summary_not_modified(self, orchestrator, mock_chroma_store, biscuit_event):
        """Summary with entity but no crisis keyword should NOT be modified."""
        mock_chroma_store.query_collection.return_value = [
            _make_doc("s2", "Biscuit is a black cat who purrs intensely"),
        ]
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_entity_not_in_content_skipped(self, orchestrator, mock_chroma_store, biscuit_event):
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_staleness_ratio_uses_max(self, orchestrator, mock_chroma_store, biscuit_event):
        """Should not overwrite a higher existing staleness_ratio."""
        mock_chroma_store.query_collection.return_value = [
            _make_doc("s4", "Biscuit almost died in the emergency", staleness_ratio=0.85),
        ]
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_both_collections_queried(self, orchestrator, mock_chroma_store, biscuit_event):
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_multiple_matching_docs(self, orchestrator, mock_chroma_store, biscuit_event):
        """Multiple crisis documents should all get annotated."""
        crisis_docs = [
            _make_doc("s5", "Biscuit was critical in the ICU"),
            _make_doc("s6", "Biscuit DNR was signed at the emergency vet"),
            _make_doc("s7", "Biscuit purrs a lot"),  # no crisis keyword
        ]
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_doc_without_id_skipped(self, orchestrator, mock_chroma_store, biscuit_event):
        """Documents without an ID should be skipped."""
        mock_chroma_store.query_collection.return_value = [
            {"content": "Biscuit was in the ICU", "metadata": {}},  # no id
        ]
    ...
    def test_disabled_when_staleness_off(self, orchestrator, mock_chroma_store, biscuit_event):
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_resolution_date_included(self, orchestrator, mock_chroma_store, biscuit_event):
        """Metadata should include resolution_date."""
        mock_chroma_store.query_collection.return_value = [
            _make_doc("s8", "Biscuit faced a serious health crisis"),
        ]
    ...
    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_crisis_keyword_make_it(self, orchestrator, mock_chroma_store, biscuit_event):
        ...
        mock_chroma_store.query_collection.side_effect = [
            [_make_doc("s9", "in case Biscuit doesn't make it")],
            [],
        ]
    ...
    def test_entity_resolver_display_name(self, orchestrator, mock_chroma_store):
        ...
        md_updates = mock_chroma_store.update_metadata.call_args_list[0][0][2]
        assert "Biscuit" in md_updates["resolution_note"]  # Capitalized from resolver
        assert md_updates["resolution_entity"] == "Biscuit"

    @patch("config.app_config.STALENESS_ENABLED", True)
    def test_no_memory_system_is_noop(self, biscuit_event):
        """Should silently return if memory_system is None."""
        from core.orchestrator import DaemonOrchestrator
        orch = DaemonOrchestrator.__new__(DaemonOrchestrator)
        orch.memory_system = None
        orch.logger = MagicMock()
        orch._cascade_entity_resolution([biscuit_event])
        # Should not raise
...
class TestStalenessPrefix:
    ...
    def test_resolution_note_takes_priority(self):
        ...
        item = {
            "metadata": {
                "resolution_note": "Biscuit is alive (confirmed 2026-04-20)",
                "staleness_ratio": 0.65,
            }
        }
        prefix = _staleness_prefix(item)
        assert "[RESOLVED" in prefix
        assert "Biscuit is alive" in prefix
    ...
    def test_disabled_returns_empty(self):
        ...
        item = {
            "metadata": {
                "resolution_note": "Biscuit is alive",
                "staleness_ratio": 0.9,
            }
        }
        prefix = _staleness_prefix(item)
