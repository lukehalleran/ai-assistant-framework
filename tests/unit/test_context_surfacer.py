...
def _make_edge(source_id="user", target_id="dexivar", relation="medication", weight=1.0):
...
def _make_graph_memory(edges=None, nodes=None, n_nodes=50, n_edges=30):
    ...
    gm.get_context_sentences.return_value = ["User takes Dexivar", "User works at Sturdy Shelter"]
    return gm
...
class TestSurfacingModels:
    def test_domain_entity_construction(self):
        e = DomainEntity(
            entity_id="dexivar",
            display_name="Dexivar",
            domain="health",
            relation="medication",
            edge_weight=2.0,
        )
        assert e.entity_id == "dexivar"
    ...
    def test_proactive_insight_construction(self):
        ...
        i = ProactiveInsight(
            ...
            active_domain="career",
            bridged_domain="health",
            entity_ids=["sturdy_shelter", "dexivar"],
            confidence=0.8,
            generated_at=now,
        )
    ...
    def test_novelty_key_determinism(self):
        i1 = ProactiveInsight(
            ...
            entity_ids=["dexivar", "sturdy_shelter"],
        )
        i2 = ProactiveInsight(
            insight_text="B",
            active_domain="career",
...
class TestSurfacingHistory:
    def test_not_recently_shown_when_empty(self):
        ...
        try:
            ...
            assert not h.was_recently_shown("career|health|dexivar")
        finally:
            os.unlink(path)
...
class TestClassifyUserEdges:
    ...
    def test_basic_classification(self):
        edges = [
            _make_edge(target_id="dexivar", relation="medication"),
            _make_edge(target_id="sturdy_shelter", relation="works_at"),
        ]
        nodes = {
            "dexivar": _make_node("dexivar", "Dexivar"),
    ...
    def test_override_takes_precedence(self):
        ...
        edges = [_make_edge(target_id="biscuit", relation="pet")]
        nodes = {"biscuit": _make_node("biscuit", "Biscuit")}
        s = self._make_surfacer(edges, nodes)
        clusters = s._classify_user_edges()
        assert "hobbies" in clusters
        ent_ids = [e.entity_id for e in clusters["hobbies"].entities]
        assert "biscuit" in ent_ids

    def test_sibling_override(self):
        edges = [_make_edge(target_id="sam", relation="sibling_of")]
        nodes = {"sam": _make_node("sam", "Sam")}
        s = self._make_surfacer(edges, nodes)
        clusters = s._classify_user_edges()
        assert "relationships" in clusters

    def test_caching_by_edge_count(self):
        edges = [_make_edge(target_id="dexivar", relation="medication")]
        nodes = {"dexivar": _make_node("dexivar", "Dexivar")}
        s = self._make_surfacer(edges, nodes)
...
class TestIdentifyActiveDomains:
    ...
    def test_entity_resolution_finds_domain(self):
        s = self._make_surfacer_with_clusters({"dexivar": "dexivar"})
        clusters = {
            "health": DomainCluster(
                domain="health",
                entities=[DomainEntity(entity_id="dexivar", display_name="Dexivar",
                                      domain="health", relation="medication")],
            ),
            "career": DomainCluster(domain="career", entities=[]),
        }
        active = s._identify_active_domains("How is my Dexivar working?", clusters)
    ...
    def test_multi_domain_detection(self):
        ...
        active = s._identify_active_domains("My Dexivar and work stress", clusters)
        assert "health" in active
        assert "career" in active
...
class TestSynthesizeInsight:
    ...
    def _make_candidate(self, active="career", bridged="health"):
        ...
        bc = DomainCluster(domain=bridged, fact_sentences=["User takes Dexivar"])
    ...
    async def test_valid_json_response(self):
        response = json.dumps([
            {"insight": "Sleep disruption from brewery connects to Dexivar timing.",
             "confidence": 0.8, "entity_ids": ["sturdy_shelter", "dexivar"]}
        ])
        s = self._make_surfacer(response)
        cand = self._make_candidate()
        insights = await s._synthesize_insights_batch([cand], max_insights=2)
        assert len(insights) == 1
        assert "Dexivar" in insights[0].insight_text
...
class TestGenerateInsights:
    ...
    async def test_session_cache(self):
        ...
        response = json.dumps([
            {"insight": "Connection found.", "confidence": 0.8, "entity_ids": ["dexivar"]}
        ])
    ...
    async def test_full_pipeline_success(self):
        ...
        response = json.dumps([
            {"insight": "Brewery shifts disrupt Dexivar timing.", "confidence": 0.85,
             "entity_ids": ["sturdy_shelter", "dexivar"]}
        ])
        s = self._make_full_surfacer(
            llm_response=response,
